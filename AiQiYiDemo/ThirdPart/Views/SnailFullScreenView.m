...
//        [self addSubview:_advertisementView];
	  ...
	  [self addSubview:_effectView];
        ...
        [_closeIcon setImage:[UIImage imageNamed:@"add_icon"] forState:UIControlStateNormal];
...
- (void)setContent {
    ...
    */
    ...
    _closeButton.size = CGSizeMake([UIScreen width], KTabBarHeight);
 _closeButton.backgroundColor = [UIColor colorWithWhite:0 alpha:0];
    _closeButton.bottom = [UIScreen height];
    _closeIcon.size = CGSizeMake(30, 30);
 _closeIcon.backgroundColor = [UIColor colorWithWhite:0 alpha:0];
 _closeIcon.center =CGPointMake(_closeButton.centerX, _closeButton.centerY-px_scale(30.0f)); //_closeButton.center-30.0f;
...
- (void)commonInitialization
{
    ...
    _scrollContainer.backgroundColor = [UIColor colorWithWhite:1 alpha:0];
    ...
    { // 设置item的默认宽高
        _itemSize.width = px_scale(150);
        _itemSize.height = px_scale(150.0f);
    }
    _gap = 15;
    ...
    _scrollContainer.bottom = [UIScreen height] - KTabBarHeight;
    _scrollContainer.contentSize = CGSizeMake(_pages * [UIScreen width], _scrollContainer.height);
 _effectView.frame = CGRectMake(0, _scrollContainer.y, [UIScreen width], [UIScreen height]-_scrollContainer.y);
  ...
  if (finished) {
	if (recognizer.view.tag == _rows * _rowCount - 1) {
	 ...
	 if ([_delegate respondsToSelector:@selector(fullScreenView:didClickItemAtIndex:)]) {
	  [_delegate fullScreenView:ws didClickItemAtIndex:recognizer.view.tag];
	 }
	}
  }
 }];
...
- (void)endAnimationCompletion:(void (^ __nullable)(BOOL finished))completion {
    ...
    [_itemViews enumerateObjectsUsingBlock:^(SnailImageLabel *item, NSUInteger idx, BOOL * _Nonnull stop) {
        [UIView animateWithDuration:0.25 delay:0.03 * (_itemViews.count - idx) options:UIViewAnimationOptionLayoutSubviews animations:^{
            item.transform = CGAffineTransformMakeTranslation(0, _rows * _itemSize.height);
            item.alpha = 0;
			if (idx==0)
			{
			 _effectView.backgroundColor = [_effectView.backgroundColor colorWithAlphaComponent:0];
			 self.backgroundColor = [self.backgroundColor colorWithAlphaComponent:0];
			self.scrollContainer.backgroundColor = [self.scrollContainer.backgroundColor colorWithAlphaComponent:0];
			}
